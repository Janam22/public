    ...
    public function fetchDataAndStore()
    {
        ...
        if (!$apiConfig || !$apiConfig->api_url) {
            Log::error('API URL not found.');
                return redirect()->route('backend.products.index')->with('error', 'An unexpected error occurred. Please try again.');
        }
        ...
        try {
            ...
            foreach ($data['data'] as $item) {
                $productData = [
                    ...
                    'product_location' => $item['rackNumber'] ?? null
                ];
