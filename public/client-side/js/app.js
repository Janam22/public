...
const lazyLoadDivImage = ()=>{
    const lazyElements = document.querySelectorAll('.lazyDiv');

    const loadBackground = (element) => {
        const bgUrl = element.getAttribute('data-bg');
        element.style.backgroundImage = `url(${bgUrl})`;
        element.classList.remove('lazy'); // Remove the lazy class if needed
    };
            ...
            const imageObserver = new IntersectionObserver((entries, observer) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        ...
                        img.onload = () => img.removeAttribute('data-src'); // Optional: remove attribute after loading
...
(function ($) {
    ...
    RESHOP.shopCategoryToggle = function() {
        ...
        $('.shop-w__toggle').on('click', function () {
            var target = $(this).data('target');

            // Hide all other open sections except the current one
            $('.shop-w__wrap.show').not(target).collapse('hide');
